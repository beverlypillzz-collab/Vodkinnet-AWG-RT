...
2. Генерирует `.env` со случайными `POSTGRES_PASSWORD` и `JWT_SECRET` (если `.env` уже существует — переиспользует его, не трогает)
3. Собирает и поднимает `postgres`, `redis`, `panel` (миграции Alembic накатываются автоматически при старте контейнера панели)
